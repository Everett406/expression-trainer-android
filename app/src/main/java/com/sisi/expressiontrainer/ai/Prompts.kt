...
object Prompts {
    ...
    fun getRealtimePrompt(text: String, context: FeedbackContext?, customPrompt: CustomPrompt?): PromptPair {
        val elapsedMin = (context?.elapsedSec ?: 0) / 60
        val topic = context?.topic ?: ""
        val prevPoints = context?.previousPoints ?: emptyList()

        val contextBlock = buildString {
            if (elapsedMin > 0) append("[已说${elapsedMin}分钟] ")
            if (topic.isNotEmpty()) append("[开头主题: \"$topic\"] ")
            if (prevPoints.isNotEmpty()) append("[已说过的观点: ${prevPoints.joinToString(";")}]")
        }

        var system = """你是中文口语表达的实时教练。每次只输出1条提示，不超过8个字，不加标点，不解释。
...
- 如果都没触发，输出一个空行
- 不管错别字、不管语音识别错误""".trimIndent()

        val customBlock = buildString {
            if (!customPrompt?.goals.isNullOrBlank()) {
                append("\n\n## 用户训练目标(调整你的反馈优先级)\n${customPrompt?.goals}")
            }
            if (!customPrompt?.customRules.isNullOrBlank()) {
                append("\n\n## 用户自定义规则(和上面的规则一起生效,触发时一样只输出1条提示)\n${customPrompt?.customRules}")
            }
            if (!customPrompt?.styleRef.isNullOrBlank()) {
                append("\n\n## 用户想要的表达风格(反馈时以此为标准)\n${customPrompt?.styleRef}")
            }
            if (!customPrompt?.customWords.isNullOrBlank()) {
                append("\n\n## 用户额外口癖词(视为填充词,出现时标记)\n${customPrompt?.customWords}")
            }
        }

        if (customBlock.isNotBlank()) {
            system += customBlock
        }
    ...
    fun getReportPrompt(fullText: String, stats: ReportPromptStats, customPrompt: CustomPrompt?): PromptPair {
        var system = """你是专业中文表达教练,融合了两套核心能力:
...
语气要求:直接、犀利、有建设性。像一个严格但真心关心你的教练。不要客套、不要废话。""".trimIndent()
...
数据:${stats.duration}秒 | ${stats.totalWords}字 | 填充词${stats.fillers}次 | 犹豫词${stats.hedges}次 | 笼统词${stats.vagueWords}次""".trimIndent()
        )
    }
    ...
    data class FeedbackContext(
        ...
        val topic: String = "",
        val previousPoints: List<String> = emptyList()
    )

    data class ReportPromptStats(
        val duration: Int,
        val totalWords: Int,
        val fillers: Int,
        val hedges: Int,
